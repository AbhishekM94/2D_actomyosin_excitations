"""
One variable Nonlinear Advection_Diffusion equation in a rectangular grid with Periodic Boundary 
conditions.
ddt(u) + c.(ddx + ddy)u = D*(d2dx2 + d2dy2)u  
...
px = Constant(0.5)
py = Constant(0.5)
ic = Expression("2*exp(-((x[0]-px)*(x[0]-px) + (x[1]-py)*(x[1]-py))/0.05) + 0.1", degree = 1, px=px, py=py)
...
F = (1.0/dt)*inner(u-u0,v)*dx + theta*adv_term(u,v) + (1.0-theta)*adv_term(u0,v) - ( theta*diff_term(u,v) + (1.0-theta)*diff_term(u0,v) )   
